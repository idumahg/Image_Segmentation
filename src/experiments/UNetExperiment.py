...
class UNetExperiment:
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            # 
            data = batch['image'].to(dtype=torch.float32, device=self.device)
            target=batch['seg'].to(dtype=torch.long, device=self.device)
    ...
    def run_test(self):
        ...
        for i, x in enumerate(self.test_data):
            ...
            def Jaccard3d(a, b):
                """
                Compute the Jaccard Index (Intersection over Union) for 3D binary masks.

                Args:
                    a (numpy array): First binary mask.
                    b (numpy array): Second binary mask.

                Returns:
                    float: Jaccard index (IoU), or -1 if both volumes are empty.
                """
                a, b = (a > 0).astype(int), (b > 0).astype(int)
                intersection = np.logical_and(a, b).sum() 
